    ...
    try {
        ...
        if (req.user.rol === "superadmin") {
            ...
            if (!institutionId)
                return res
                    .status(403)
                    .json({ error: "Este admin no tiene una institución asignada" });
            resultado = await StudentModel.obtenerPorInstitucion(institutionId, {
                search,
                limit,
                offset,
            });
...
export async function obtenerUno(req, res) {
    try {
        ...
        const estudiante = await StudentModel.obtenerPorId(
            req.params.id,
            institutionId,
        );
        if (!estudiante) {
...
export async function crear(req, res) {
    ...
    if (!people_id || !status_id || !grade_id || !start_date) {
        return res
            .status(400)
            .json({
                error: "people_id, status_id, grade_id y start_date son requeridos",
            });
    }
    ...
    try {
        ...
        if (error.code === "23505") {
            return res
                .status(409)
                .json({ error: "Esta persona ya tiene un perfil de estudiante" });
        } else if (error.code === "23503") {
            return res
                .status(404)
                .json({ error: "people_id, grade_id o status_id no existen" });
        } else if (error.code === "23514") {
            return res
                .status(400)
                .json({ error: "end_date no puede ser anterior a start_date" });
        }
...
export async function actualizar(req, res) {
    ...
    if (!status_id || !grade_id || !start_date) {
        return res
            .status(400)
            .json({ error: "status_id, grade_id y start_date son requeridos" });
    }
    ...
    try {
        ...
        if (req.user.rol === "superadmin") {
            ...
            actualizado = await StudentModel.actualizar(
                req.params.id,
                institutionId,
                {
                    status_id,
    ...
    try {
        ...
        if (error.code === "23503") {
            return res
                ...
                .json({
                    error:
                        "No se puede eliminar ya que estudiante tiene registros asociados (campañas, actualizaciones, etc...)",
                });
        }
...
export async function campanasElegibles(req, res) {
    try {
        const institutionId =
            req.user.rol === "superadmin" ? null : req.user.institution_id;
